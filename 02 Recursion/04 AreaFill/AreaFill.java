...
public class AreaFill
{
   ...
   public static char[][] read(String filename)
   {
      ...
      int rowCount = 0;
      String param = infile.nextLine().trim();
      String [] sepParam = param.split(" ");
      int rowNum = Integer.parseInt(sepParam[0]);
      int colNum = Integer.parseInt(sepParam[1]);
      char[][] finalReturn = new char[rowNum][colNum];
      while(infile.hasNext())
      {
         String preSplit = infile.nextLine();
         char[] row = preSplit.toCharArray();
         finalReturn[rowCount] = row;
         rowCount++;
      }
      return finalReturn;
   }
   ...
   {
      if(r >= g.length|| c >= g[0].length|| c < 0 || r < 0)
         return 0;
      else if(g[r][c] == ch)
      {
         g[r][c] = '*';
